...
//ofEnableAlphaBlending();
	...
	//基础设置
	//gui = new ofxUICanvas(200,0,400,600);//不加参数 就是自适应 加了就是那个大小 超过的会被裁剪
	...
	//自制取色器
	colorPicker = new ofxUICanvas(800,0,400,600);
	colorPicker->addLabel("MyProject2", OFX_UI_FONT_LARGE);//大标题
	colorPicker->addFPSSlider("FPS SLIDER",200, 50, 100);//帧率检测
	colorPicker->addLabel("colorPicker");
	colorPicker->addSpacer();
	red = 0;
	green = 0;
	blue = 0;
	alpha = 255;
